...
public class Player : MonoBehaviour
{
    ...
    void UpdateRayInput()                                                                   //레이캐스트 관련함수
    {
        ...
        if (Physics.Raycast(ray, out hit, m_MaxDistance))                               //광선이 앞으로 나아가며 MaxDistance만큼의 길이를 가지고 있다.
        {
            ...
            else if((hit.collider.CompareTag("Key") && hit.collider.GetComponent<MeshRenderer>().enabled == true) || hit.collider.CompareTag("FlashLight"))
            {
            ...
            if (Input.GetMouseButtonDown(0))                                                    //왼클 했을 경우
            {
                if (hit.collider.CompareTag("Door"))                                        //태그가 도어라면 (광선이 태그에 맞았다면)
                {
                    ...
                    Door autodoor = hit.collider.GetComponent<Door>();
                    if (door == true || autodoor == true)                                                                //잘 불러와졌고
                    {
                        if (door != null && door.canOpen == true)                                            //열수 있는 상태라면
                        {
                            //text.UpdateState(UIManager.TextState.DoorOpenAndClose);
                        ...
                        else if(autodoor != null && autodoor.canOpen == true)
                        {
                            m_JoDoor = autodoor;
						}
                        else                                                                //열수 없는 상태라면
